...
i18n.configure({
  ...
  defaultLocale: 'en',
  // Más configuraciones según tus necesidadess
});
...
app.use(cors({
  origin: ['https://qryptogenia.disruptiveinfotech.com', 'http://localhost:5173'],
...
app.post('/api/set-language', (req, res) => {
  ...
  if (['en', 'es'].includes(language)) {
    // Aquí puedes guardar el idioma en una base d  e datos o en algún otro lugar
...
app.get('/api/some-endpoint', (req, res) => {
  ...
  i18n.setLocale(language);
  const message = i18n.__('welcomeMessage'); // Ejem  plo de uso de traducción
