  ...
  try {
    ...
    const newUser = await prisma.user.create({
      data: {
        ...
        rol: { connect: { id: 2 } },
        authProvider: 'email', // Asegúrate de incluir esto si el registro es por Google
...
export const googlecall = async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      user = await prisma.user.create({
        data: {
          ...
          rol: { connect: { id: 2 } },
          authProvider: 'google', // Agregar el campo aquí
        },
        include: {
          rol: true,
        },
    ...
    } else if (user.rol.name === "CLIENT") {
      const findUser=await prisma.membershipUser.findFirst({where:{userId:user.id}});
      if(findUser){
      return res.redirect(`http://localhost:5173/user/home?token=${token}`);
      }else{
        return res.redirect(`http://localhost:5173/selectPlan?token=${token}`);
      }
    } else {
