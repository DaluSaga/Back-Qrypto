...
const getQrUserController = async (req, res, next) => {
    try {
      ...
      const restantes = membershipUser.membership.scan_qrs;
  
        const qrCodesWithBase64 = qrCodes.map(qrCode => ({
            ...qrCode,
            qr_image_base64: qrCode.qr_image_base64.toString('base64'),
            scan_qrs: restantes - qrCode.scan_count,
        }));
  console.log(qrCodesWithBase64)
...
const getMembership = async (req, res) => {
  ...
  const getUniqueKey = async (req, res) => {
    const { uniquekey } = req.params;

    try {
      // Consulta en la base de datos para ver si el uniquekey ya existe
      const existingRecords = await prisma.qr.findMany({
          where: { qr: uniquekey },
      });

      // Si el array tiene al menos un elemento, significa que el uniquekey ya existe
      if (existingRecords.length > 0) {
          return res.json({ exists: true });
      } else {
          // Si el array está vacío, significa que el uniquekey no existe
          return res.json({ exists: false });
      }
  } catch (error) {
      console.error('Error checking unique key:', error);
      return res.status(500).json({ message: 'Error checking unique key.' });
  }
};
