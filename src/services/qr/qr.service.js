...
class QrService {
  ...
  async createQr(qrData, userId) {
    ...
    try {
      const newQR = await prisma.$transaction(async (prisma) => {
        ...
        const qrDataValue = qrType.type !== "website-url" && qrType.type !== "pdf" && qrType.type !== 'wifi' ? 
  this.generateQrDataValue(qrType, qrData, newQR.id) : 
  qrData.qr.url;
        const updatedQR = await prisma.qr.update({
          where: { id: newQR.id },
          data: { qr: qrData.qr.data, url: qrDataValue},
        });
      ...
      const relatedData = await prisma.$transaction(async (prisma) => {
        ...
        if(qrType.type=="food-menu"){
          menuPreview=await this.qrPreviewService.createQrMenuPreview(qrData.qrPreview,newQR.id);
  ...
  generateQrDataValue(qrType, qrData, qrId) {
    ...
    if (qrType.type !== "website-url" && qrType.type !== "pdf" && qrType.type!=="wifi") {
